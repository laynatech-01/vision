...
let bloqueoBienvenida = false; // Nueva variable de control
...
function hablar(texto, urgente = false, esBienvenida = false) {
    if (bloqueoBienvenida && !esBienvenida) return; // Si está hablando la bienvenida, ignora lo demás
    if (!modoDescripcion && !urgente) return;
    
    if (urgente && !bloqueoBienvenida) window.speechSynthesis.cancel();
    ...
    if (esBienvenida) {
        bloqueoBienvenida = true;
        msg.onend = () => { bloqueoBienvenida = false; }; // Libera el sistema al terminar
    }
...
function detenerSistema() {
    streaming = false;
    bloqueoBienvenida = false; // Resetear bloqueo por seguridad
